 ...
 */
@Repository
public class AutohausRepositoryDeprecated {
    private static final Logger LOGGER = LoggerFactory.getLogger(AutohausRepositoryDeprecated.class);

    /**
     * Erstellt eine Instanz der AutohausRepository-Klasse.
     */
    public AutohausRepositoryDeprecated() {
    }
     ...
     *
     * @param suchkriterien MultiValueMap mit Suchparametern wie "name" und "stadt".
    ...
    public @NonNull List<Autohaus> get(@NonNull final Map<String, List<String>> suchkriterien) {
        ...
        for (final var entry : suchkriterien.entrySet()) {
            switch (entry.getKey()) {
                ...
                case "stadt" -> {
                    return getByStadt(entry.getValue().getFirst());
                }
                default -> {
    ...
    /**
     * Sucht Autohaeuser anhand des stadts.
     *
     * @param stadt der stadt des gesuchten Autohauses.
     * @return Liste von Autohaeusern, deren stadt den angegebenen String enthält.
     */
    public @NonNull List<Autohaus> getByStadt(final String stadt) {
        LOGGER.debug("getByStadt: stadt={}", stadt);
        final var autohaeuser = AUTOHAEUSER.stream()
            .filter(autohaus -> autohaus.getAdresse().getStadt().contains(stadt))
            .toList();
        LOGGER.debug("getBystadt: autohaeuser={}", autohaeuser);
        return autohaeuser;
    }
    ...
    public @NonNull Optional<Autohaus> getById(@NonNull final String id) {
        ...
        final Optional<Autohaus> autohaus = AUTOHAEUSER.stream()
            .filter(a -> Objects.equals(id, a.getId().toString()))
            .findFirst();
    ...
    public void update(final @NonNull Autohaus autohaus) {
        ...
        final OptionalInt index = IntStream
            ...
            .filter(i -> Objects.equals(AUTOHAEUSER.get(i).getId(), autohaus.getId()))
            .findFirst();
